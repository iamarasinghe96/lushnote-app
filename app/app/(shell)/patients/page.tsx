...
export default function PatientsPage() {
  ...
  useEffect(() => {
    function onReselect(e: Event) {
      ...
      if (detail?.href === '/app/patients') setSelectedPatient(null)
    }
  ...
  function loadNote(note: Note) {
    ...
    if (note.docType === 'letter' && note.id) {
      router.push('/app/edit?noteId=' + note.id)
      return
    }
  ...
  function startClinicalNoteFromHistory(template: AnyTemplate, noteLength: NoteLength) {
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <>
        ...
        {unfinishedDrafts.map(d => (
          <div
            key={d.id}
            onClick={() => router.push(`/app/generate?recover=1&draft=${encodeURIComponent(d.id)}`)}
