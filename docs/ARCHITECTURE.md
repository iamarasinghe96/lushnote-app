# LushNote - architecture and reasoning

> **This file is REFERENCE, not rules.** The rules an agent must follow are in
> [`AGENTS.md`](../AGENTS.md) - read that first. This explains how the app works
> and, more usefully, WHY it works the way it does. Most sections were written
> after something failed in a clinic.
>
> **Never copy code into this file.** A pasted function drifts from the real one
> and then misleads with confidence. Name the file instead.
...
A map of where things live, deliberately NOT a file list. The list that used to be
here named 4 of 20 API routes and 5 of 55 `lib` modules, and omitted `hooks/` and
`scripts/` entirely - while a rule said not to create files outside it, which
between them forbade touching most of the live codebase.
...
| `lib/` | Everything with no UI. Pure modules (`entitlement`, `aiCost`, `captureIntent`, `workplaceType`) sit beside provider adapters (`gemini`, `groq`, `billing`) |
| `lib/firestore/` | All database access. Client SDK and `-admin` variants are separate: the client SDK cannot authenticate inside a serverless function |
| `hooks/` | React hooks, including the recorder, the note store and the support thread |
| `tests/unit/` | Pure modules and policy. `tests/rules/` runs against the Firestore emulator |
| `scripts/` | Dev tooling, not shipped. `dashScan.mjs` backs the em-dash test |
| `docs/` | This file |

Root: `firestore.rules`, `next.config.mjs` (the CSP lives here), `vercel.json`,
and the planning documents listed in `AGENTS.md`.
...
**The rules live in `firestore.rules` at the repo root. That file is the only
copy.** Deploy with `firebase deploy --only firestore:rules`.

A full copy used to be pasted here, alongside an instruction to paste it into the
Firebase console. It had drifted: it was missing `templateId`/`templateName` from
the note allow-list, four `profileValid()` fields, and the whole `/appearance`
block, and carried an older `noPrivilegeEscalation()`. Pasting it would have
rejected every new note write and every new signup. `SECURITY_NOTES.md` then
audited the copy rather than the real file, so the drift was invisible to both.

Each subcollection needs its OWN `match` block - Firestore rules do NOT cascade
from `users/{uid}` to `users/{uid}/transcriptDrafts/...`, so a missing block means
the catch-all `allow read, write: if false` silently denies every access.
...
**Unit tests cover the pure modules only**, and there are now 30 of them plus a
Firestore rules suite. Several pin POLICY rather than a function - see the table
in `AGENTS.md`. Anything needing Firestore or Stripe is
...
change) and read only via `app/api/admin/logs`. Retention: a Firestore TTL on `system_logs.expiresAt`, which `logToSink`
writes as a FUTURE timestamp 90 days out. **Never point a TTL at `createdAt`** -
a TTL deletes a document once its field is in the PAST, and `createdAt` always
is, so that policy would delete every log within a day.
...
Resets on a new US-Pacific date via `quotaDate()`, matching Google's own reset.

---

## Pro tier: LushNote's own paid AI keys

A paying doctor stops managing API keys. Everyone else keeps their own, which is
what makes the upgrade concrete and holds trial API cost at zero.
...
`PRO_STATES` (`lib/entitlement.ts`) is `active`, `dunning`, `paused`, `exempt`.
`dunning` is included because a card retry or a clearing BECS debit is money
already in flight; `exempt` because a complimentary account means the full product.
...
`generate`, `transcribe`, `chat` and `ocr` each call `requireUser(req)`
(`lib/adminGuard.ts`) before reading any provider key, and bill the VERIFIED
doctor. The uid used to arrive in the request body and was checked only for
length, against public endpoints - survivable while the routes only ever spent the
doctor's own key, not survivable once a route can spend ours.

Client-side, `lib/aiHeaders.ts` attaches the token and any keys the doctor has. It
is client-only and deliberately not in `lib/utils.ts`, which server code imports -
putting the Firebase client SDK there would drag it into a server bundle.

**Two call sites post `FormData`** (the segmented recorder, the ward-note scanner)
and must pass `json: false`. A hand-set `Content-Type` corrupts a multipart upload,
and one of those two is the transcription path.
...
each of which under-reports silently rather than failing loudly, and each pinned by
a test: audio must be priced at the audio rate and not billed twice (Gemini reports
it inside `promptTokenCount`), `thoughts` tokens are billed and appear in neither
other field, an unknown model returns null rather than 0, and the arithmetic stays
in integer micro-USD because floats drift over a month of calls.

## Smaller things worth knowing

- **`lib/adminSections.ts`** holds the admin nav. Five sections are pinned, the
  rest sit under More; the split is pinned by a test. It lives outside
  `app/admin/page.tsx` so it can be read without importing eleven panels.
- **`hooks/useUrlTab.ts`** keeps a tab selection and the address bar in step.
  `replaceState`, not `pushState`: back still means "leave this page".
- **`components/ui/BackButton.tsx`** is the one back control. A literal arrow
  character is not an icon - it renders at the font's weight and gives a target no
  bigger than the glyph.
- **The `.ln-glass` family** (`app/globals.css`) is the glass system:
  `-light`, `-modal`, `-brand`, `-note`, `-toolbar`, `-tabbar`, `-pill`. Its tint
  and frost live on `::before`/`::after` at NEGATIVE z-index, so **a host must
  establish a stacking context** or both layers leak behind an ancestor's
  background. Use these rather than hand-rolling `backdrop-filter`.
...
## AI assistant patient recall

A function called `lnRecallSearch` was documented here. It does not exist. The
assistant builds its context with `buildNotesContext(question, notes)` in
`components/FAB.tsx` - keyword excerpt windows over eight note fields, a stop-word
filter, capped snippets per note and an overall character cap.
...
User menu in header has 8 labelled links to `/settings?tab={tabId}`:
...
## Gemini usage and AI cost

`incrementGeminiUsage` was documented here with a full implementation. **No such
function has ever existed.** An agent told to write it would have bypassed the
cost meter entirely, so Pro-tier spend on LushNote's paid key would go uncounted.

The real path, in `lib/firestore/profiles-admin.ts`:

- `meterGemini(uid, modelKey, usage, audioTokens)` - what the API routes call. It
  runs the quota counter and the cost meter together, in two transactions, so the
  two cannot drift apart at a call site.
- `updateGeminiUsage` - the quota counter, `users/{uid}.geminiUsage`.
- `recordAiSpend` - the cost meter, `users/{uid}.aiCost`, keyed by month.

Pricing is `lib/aiCost.ts`, which is pure and carries an explicit warning that
its rates are estimates not verified against the providers' pricing pages.

The day key is `quotaDate()` (`lib/utils.ts`), which is **US Pacific**, not UTC -
Google's free tier resets on Pacific midnight, and using UTC would diverge from
