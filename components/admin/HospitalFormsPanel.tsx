...
export default function HospitalFormsPanel() {
  const { user } = useAuth()
  ...
  async function call(body: Record<string, unknown>) {
    ...
    const res = await fetch('/api/admin/hospital-form', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(body) })
  ...
  async function fetchAll() {
    ...
    try { const r = await call({ action: 'listForms' }); setForms((r as { forms: HospitalFormDoc[] }).forms) }
    catch (e) { setError(e instanceof Error ? e.message : 'Failed to load') } finally { setFetching(false) }
  }

  function addCampus() { const v = campusInput.trim(); if (!v) return; if (!campuses.includes(v)) setCampuses(prev => [...prev, v]); setCampusInput('') }
  ...
  async function pickPage(idx: number, file: File) { const dataUrl = await readFileAsDataUrl(file); setPages(prev => prev.map((p, i) => i === idx ? dataUrl : p)) }
  function resetForm() { setName(''); setCampuses([]); setCampusInput(''); setGeometryText(JSON.stringify(DEFAULT_GEOMETRY, null, 2)); setPages([null, null]); setDateColLabel('Date / Time'); setNotesColLabel('Please sign each entry and print surname and designation') }
  ...
  async function handleUpload() {
    ...
    setUploading(true)
    try { await call({ action: 'upload', name: name.trim(), organizationNames: campuses, geometry, labels: { dateCol: dateColLabel, notesCol: notesColLabel }, pageDataUrls: pages }); setToast('Form saved'); resetForm(); fetchAll() }
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="rounded-2xl p-5 space-y-4" style={CARD}>
        ...
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. AWH Progress Notes (FAW0004)" className="mt-1 w-full text-sm border border-[var(--border)] rounded-lg px-3 py-2 bg-white outline-none focus:border-[#2563eb]" />
        </div>
        ...
        <div>
          ...
          {campuses.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {campuses.map(c => <span key={c} className="inline-flex items-center gap-1 text-xs bg-[#eff6ff] text-[#2563eb] rounded-full px-2 py-1">{c}<button onClick={() => setCampuses(prev => prev.filter(x => x !== c))} aria-label="Remove">✕</button></span>)}
            </div>
          )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs font-medium text-[#475569]">Date column label</label><input value={dateColLabel} onChange={e => setDateColLabel(e.target.value)} className="mt-1 w-full text-sm border border-[var(--border)] rounded-lg px-3 py-2 bg-white outline-none focus:border-[#2563eb]" /></div>
          <div><label className="text-xs font-medium text-[#475569]">Notes column label</label><input value={notesColLabel} onChange={e => setNotesColLabel(e.target.value)} className="mt-1 w-full text-sm border border-[var(--border)] rounded-lg px-3 py-2 bg-white outline-none focus:border-[#2563eb]" /></div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-1">
            {pages.map((p, i) => <PageSlot key={i} idx={i} preview={p} onPick={pickPage} onClear={() => setPages(prev => prev.map((x, j) => j === i ? null : x))} />)}
          </div>
        </div>
